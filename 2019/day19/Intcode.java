...
public class Intcode {
	...
	String inps = (input.size() > 0) ? input.get(0)+"" : "empty";
	IO.print("ROBOT || state = " + state + " | input = " + inps + " | output = " + output);
    }

    public void setInput() {
	if (input.size() == 0) {
	    IO.print("ERROR! NO INPUT AVAILABLE!");
	} else {
	    Long i = input.remove(0);
	    IO.print("Setting input: " + i);
	...
	case 4:
	    ...
	    output = par1;
	    accOutput += output;
	    if (displayOutput) System.out.format("%1$-1c", (char) output);
